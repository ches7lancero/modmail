...
class onMessage(commands.Cog):
  ...
  async def on_message(self, message):
    ...
    if isinstance(message.channel, discord.DMchannel):
      guild = self.bot.get_guild(753824401451122741)
      categ = utils.get(guild.categories, name = "Modmail tickets")
      if not categ:
        overwrites = {
          guild.default_role : discord.PermissionOvewrites(read_messages = False),
      ...
      embed.set_author(name v message.author, icon_url = message.author.avatar_url)
      await channel.send(embed v embed)
